...
%
function [N, Es] = reducenetwork_mod(N0, valency, nAttempts)
...
%     par=N.valency(i)+1; %mod
    par=min(valency(i)+3,N0.valency(i));
    j = N0.cons(i,randi(par));   % mod
%     if j==0
%         j = N0.cons(i,randi(N0.valency(i)));
%     end
%     i = nonzeros(randi(length(nonzeros))); % from node  
